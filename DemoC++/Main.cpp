...
static std::wstring ShortenString(std::wstring s_String, int s32_MaxLines);
...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    case 1:
        e_Demo        = Demo_SMTP;
        e_Security    = cCommon::Secur_SSL;  // requires SSL or TLS!
    ...
    case 2:
        e_Demo        = Demo_POP3;
        e_Security    = cCommon::Secur_SSL; // requires SSL!
    ...
    case 6:
        e_Demo        = Demo_SMTP;
        e_Security    = cCommon::Secur_TLS_opt;
    ...
    case 8:
        e_Demo        = Demo_SMTP;
        e_Security    = cCommon::Secur_TLS_opt;
        u16_From      = L"John Miller <jmiller@hotmail.com>";
        u16_To        = L"John Miller <jmiller@hotmail.com>";
        u16_Server    = L"smtp-mail.outlook.com";
        u16_User      = L"jmiller@hotmail.com";      
        u16_Password  = L"TopSecret";
        break;

    case 9:
        e_Demo        = Demo_POP3;
        e_Security    = cCommon::Secur_SSL;
        u16_From      = NULL; // not used
        u16_To        = NULL;
        u16_Server    = L"pop-mail.outlook.com";
        u16_User      = L"jmiller@hotmail.com";      
        u16_Password  = L"TopSecret";
        break;
    ...
    case 10:
        ...
        u16_From      = NULL; // not used
        u16_To        = NULL;
        u16_Server    = L"imap-mail.outlook.com";
        u16_User      = L"jmiller@hotmail.com";      
        u16_Password  = L"TopSecret";
        break;
    ...
    if (e_Security < cCommon::Secur_Normal || 
        e_Security > cCommon::Secur_TLS_force)
    {
        PrintW(RED, L"You must assign a valid value to e_Security");
        getch();
        return 0;
    }

    if (wcsstr(u16_User, L"miller") || wcsstr(u16_User, L"Miller"))
    {
        PrintW(RED, L"Before compiling you must enter a valid user, email and password.");
        getch();
        return 0;
    }
    ...
    std::wstring s_ExePath = u16_Dir;
    int s32_Pos = s_ExePath.find(L"\\output");

    std::wstring s_RootDir = s_ExePath.substr(0, s32_Pos);
    ...
    try
    {
        switch (e_Demo)
        {
            case Demo_SMTP:
            {
                ...
                std::wstring s_Attach = s_RootDir + L"\\libvmime\\doc\\Readme.txt";
                std::wstring s_Embed  = s_RootDir + L"\\VmimeLogo.png";
            ...
            case Demo_IMAP:
            {
                ...
                std::vector<std::wstring> i_FolderList;
...
void PrintMailToConsole(cEmailParser* pi_Email)
{
    ...
    std::wstring s_Subject = pi_Email->GetSubject();
    PrintW(YELLOW, L"Email %u: Subject='%s'", u32_Email, s_Subject.c_str());

    std::wstring s_FromName; 
    std::wstring s_FromEmail = pi_Email->GetFrom(&s_FromName);
    ...
    std::wstring s_UID = pi_Email->GetUID();            // POP3: sends here the UIDL command
    ...
    std::wstring s_Plain = pi_Email->GetPlainText();    // POP3: sends here the RETR command, IMAP: sends FETCH BODY command
    PrintW(YELLOW, L"Email %u: PlainText=", u32_Email);
    PrintW(CYAN,   L"%s", ShortenString(s_Plain, 20).c_str());

    std::wstring s_Html = pi_Email->GetHtmlText();
    ...
    std::wstring s_ObjId, s_ObjType;
    std::string  s_ObjData;
    ...
    std::wstring s_AttName, s_AttType;
    std::string  s_AttData;
