...
SMTPChunkingOutputStreamAdapter::SMTPChunkingOutputStreamAdapter(ref <SMTPConnection> conn,
    utility::progressListener* progress, utility::stream::size_type size)
	: m_connection(conn), m_bufferSize(0), m_chunkCount(0)
{
    // FIX by Elmue: Added progress output
    m_progress  = progress;
    m_totSize   = size;
    m_sentBytes = 0;

    if (m_progress)
        m_progress->start(m_totSize);
}
	...
	(const value_type* const data, const size_type count, const bool last)
{
    ...
    if (m_progress)
        m_progress->progress(m_sentBytes, std::max(m_sentBytes, m_totSize));
