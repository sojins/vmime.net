...
array<String^>^ Imap::EnumFolders()
{
    try
    {
        std::vector<std::wstring> i_Folders;
        mpi_Imap->EnumFolders(i_Folders);

        array<String^>^ s_Array = gcnew array<String^>(i_Folders.size());

        for (size_t i=0; i<i_Folders.size(); i++)
        {
            s_Array[i] = gcnew String(i_Folders.at(i).c_str());
        }
...
String^ Imap::GetCurrentFolder()
{
    try
    {
        return gcnew String(mpi_Imap->GetCurrentFolder().c_str());
    }
    catch (std::exception& Ex)
    {
        throw gcnew Exception(GCSTRING(Ex.what()));
...
EmailParser^ Imap::FetchEmailAt(int s32_Index)
{
    ...
    {   
        ...
        return gcnew EmailParser((IntPtr)p_Email);
    }
    catch (std::exception& Ex)
    {
        throw gcnew Exception(GCSTRING(Ex.what()));
